...
const createNTimesSixPhotoSets = (n, round = 0) => {
  ...
  for (var i = initial; i < end; i++ ) {
    for (var j = 0; j < 6; j++ ) {
      ...
      photoEntryRow = photoEntryRow + photos[randomizeInt(photos.length - 1)] + ',' + newDate + ',' + randomWords[randomizeInt(randomWords.length - 1)] + ',' + getRandomInt(initial, end) + ',' + `${i + 1}` + '\n';
    }
  }
...
const generateSixtyMillionPhotoSets = (n = 0) => {
  ...
  if ( n === 0) {
    t0 = Date.now();
    fs.writeFile(`photos${n}.csv`, createNTimesSixPhotoSets(1000000, 0), (err) => {
      if (err) throw err;
    ...
    fs.appendFile(`photos${n}.csv`, createNTimesSixPhotoSets(1000000, n), (err) => {
      if (err) throw err;
